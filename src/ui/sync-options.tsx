...
function Options(){
  ...
  return <div className="settings-shell"><header><Brand/><span className="version-label">v{VERSION}</span></header><main className="settings-layout"><aside><h1>让收到的简历，<br/>自动进入飞书。</h1><p>每台电脑使用统一的同步服务。<br/>飞书应用密钥保留在服务端。</p></aside><section className="settings-card"><h2>自动同步设置</h2><p className="helper">首次使用：填写管理员提供的访问凭证，启用自动同步后保存。默认服务地址已填好。</p>{notice&&<div className="notice neutral" role="status">{notice}</div>}<form onSubmit={save}>
    <label className="field-label">同步服务地址<input type="url" required={s.enabled} value={s.service_origin} onChange={e=>set({...s,service_origin:e.target.value})} placeholder={SYNC_DEFAULTS.service_origin} disabled={busy}/></label>
